...
class LatentDiffusion(DDPM):
    ...
    @rank_zero_only
    @torch.no_grad()
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx): #TODO
    ...
    def p_losses(self, x_start, cond, t, noise=None, shadowmask=None, batch=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output, predicted_mask = self.apply_model(x_noisy, t, cond, batch=batch)
        ...
        gt_mask=batch['shadowmask'].unsqueeze(1)
        bce_loss_criterion = nn.BCELoss()
        dice_loss_criterion = DiceLoss()
        for predicted_mask_, gt_mask_, t_ in zip(predicted_mask, gt_mask, t):
            if t_ < 700:
                bce_loss = bce_loss_criterion(predicted_mask_.unsqueeze(0), gt_mask_.unsqueeze(0))
                dice_loss = dice_loss_criterion(predicted_mask_.unsqueeze(0), gt_mask_.unsqueeze(0))
                loss_mask = bce_loss + 0.1*dice_loss
                loss += loss_mask
